...
function overrideTags (parent, child, telegraf) {
  ...
  formatTags(child, telegraf).forEach(function (tag) {
    ...
    if (idx < 1) { // Not found or first character
      ...
      var tagName = tag.substring(0, idx);
      var tagValue = tag.substring(idx + 1);

      childCopy[tagName] = childCopy[tagName] || [];
      childCopy[tagName].push(tagValue);
    }
  });
  ...
  var result = parent.map(function (tag) {
    ...
    if (childCopy.hasOwnProperty(key)) {
      var values = childCopy[key];
      // Handle duplicate keys
      if(values.length > 1){
        duplicateKeys = duplicateKeys.concat(getDuplicateKeys(key, values));
